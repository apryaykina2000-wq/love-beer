...
// Helper: bottom-anchored parallax layer without stretching artifacts
function addParallaxLayer(scene, key, bottomY, desiredHeight){
  const tx = scene.textures.get(key);
  let imgH = 100, imgW = 100;
  if (tx && tx.frames && tx.frames.__BASE){
    imgH = tx.frames.__BASE.height;
    imgW = tx.frames.__BASE.width;
  }
  const scale = desiredHeight / imgH;
  const layer = scene.add.tileSprite(0, bottomY, WIDTH, desiredHeight, key).setOrigin(0,1);
...
class Menu extends Phaser.Scene{
  ...
  create(){
    ...
    const groundTop = HEIGHT - DISPLAY.GROUND_H + 2;
    this.far = addParallaxLayer(this, 'bg_hills_far', groundTop - 110, 180);
    this.near= addParallaxLayer(this, 'bg_hills_near', groundTop - 20, 260);
    ...
    this.add.tileSprite(0, HEIGHT, WIDTH, DISPLAY.GROUND_H, 'ground_tile').setOrigin(0,1);
    ...
    const btn = hasPlay ? this.add.image(WIDTH/2, HEIGHT-180, 'ui_btn_play').setScale(0.9)
  ...
  update(){ if(this.far){this.far.tilePositionX+=0.12;} if(this.near){this.near.tilePositionX+=0.24;} }
}
...
class Game extends Phaser.Scene{
  ...
  create(){
    ...
    // ground
    this.ground = this.add.tileSprite(0, HEIGHT, WIDTH, DISPLAY.GROUND_H, 'ground_tile').setOrigin(0,1);
    this.groundY = HEIGHT - DISPLAY.GROUND_H + DISPLAY.GROUND_RUNLINE_FROM_TOP;
    ...
    this.player.body.setGravityY(1800); // gravity down a bit
    this.player.setCollideWorldBounds(true);

    // tighter hitbox
    const pb = this.player.getBounds();
    this.player.body.setSize(pb.width*0.48, pb.height*0.82, true);
    ...
    // easier pacing
    this.speed=2.8;
    this.spawnT=0; this.spawnInterval=2100;
    this.itemT=0;  this.nextItemDelay=Phaser.Math.Between(1100,1700);
  }

  tryJump(){
    const onGround = (this.player.y >= this.groundY - 1 && this.player.body.velocity.y >= 0);
    if (onGround){
      this.player.setVelocityY(-900);  // higher jump
      this.player.play('jump', true);
      this.canDouble=true;
    } else if (this.canDouble){
      this.player.setVelocityY(-780);  // second jump
      this.player.play('jump', true);
  ...
  spawnObstacle(){
    ...
    if (this.score>=40) key = rnd<0.28?'obstacle_high':(rnd<0.62?'obstacle_mid':'obstacle_low');
    else if (this.score>=20) key = rnd<0.12?'obstacle_high':(rnd<0.52?'obstacle_mid':'obstacle_low');
    else key = rnd<0.68?'obstacle_low':'obstacle_mid';
    ...
    o.body.allowGravity=false; o.setImmovable(True=false?false:true);  // ensure immovable
    const b=o.getBounds();
    o.body.setSize(b.width*0.72, b.height*0.82, true);
  }
  ...
  update(time,delta){
    if (this.far) this.far.tilePositionX += 0.12;
    if (this.near) this.near.tilePositionX += 0.26;
    if (this.ground) this.ground.tilePositionX += this.speed*1.5;
    ...
    // speed/ease
    if (this.score < 20){ this.speed=2.8; this.spawnInterval=2100; }
    else if (this.score < 40){ this.speed=3.5; this.spawnInterval=1800; }
    else { this.speed=4.0; this.spawnInterval=1600; }
    ...
    if (this.spawnT >= this.spawnInterval){
      this.spawnT = 0;
      this.spawnObstacle();
      if (this.score >= 35 && Math.random() < 0.22){
        this.time.delayedCall(360, ()=> this.spawnObstacle());
      }
    }
...
class Result extends Phaser.Scene{
  ...
  create(){
    this.add.image(0,0,'bg_sky').setOrigin(0).setDisplaySize(WIDTH, HEIGHT);
    const groundTop = HEIGHT - DISPLAY.GROUND_H + 2;
    addParallaxLayer(this, 'bg_hills_far', groundTop - 110, 180);
    addParallaxLayer(this, 'bg_hills_near', groundTop - 20, 260);
    this.add.tileSprite(0, HEIGHT, WIDTH, DISPLAY.GROUND_H, 'ground_tile').setOrigin(0,1);
